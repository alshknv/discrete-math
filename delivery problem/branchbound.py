...
def lower_bound(g, sub_cycle):
    ...
    current_weight = sum([g[sub_cycle[i]][sub_cycle[i + 1]]['weight']
    ...
    min_to_s_weight = min([g[v][s]['weight']
                          for v in g.nodes() if v not in sub_cycle])
    # The minimum weight of an edge connecting the vertex t to a vertex from outside of sub_cycle.
    min_from_t_weight = min([g[t][v]['weight']
                            for v in g.nodes() if v not in sub_cycle])
...
def branch_and_bound(g, sub_cycle=None, current_min=float("inf")):
    ...
    if len(sub_cycle) == g.number_of_nodes():
        weight = sum([g[sub_cycle[i]][sub_cycle[i + 1]]['weight']
                     for i in range(len(sub_cycle) - 1)])
    ...
    for (d, v) in unused_nodes:
        ...
        if lower_bound(g, extended_subcycle) < current_min:
            min = branch_and_bound(g, extended_subcycle, current_min)
            if (min < current_min):
                current_min = min
